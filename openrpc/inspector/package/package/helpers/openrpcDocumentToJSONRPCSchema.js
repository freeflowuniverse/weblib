...
var openrpcDocumentToJSONRPCSchema = function (openrpcDocument) {
    return {
        ...
        properties: {
            ...
            method: {
                type: "string",
                oneOf: openrpcDocument && openrpcDocument.methods && openrpcDocument.methods.map(function (method) {
                    return {
                        const: method.name,
        ...
        allOf: openrpcDocument && openrpcDocument.methods && openrpcDocument.methods.map(function (method) {
            return {
                if: {
                    properties: {
                        method: {
                    ...
                    properties: {
                        params: {
                            oneOf: [
                                {
                                    type: "array",
                                    minItems: method.params && method.params.filter(function (param) { return param.required; }).length,
                                    maxItems: method.params && method.params.length,
                                    defaultSnippets: method.examples ? method.examples.map(function (example) {
                                        return {
                                            label: example.name,
                                            description: example.description || example.summary,
                                            body: example.params && example.params.map(function (ex) { return ex.value; }),
                                        };
                                    }) : [],
                                    items: method.params && method.params.map(function (param) {
                                ...
                                {
                                    type: "object",
                                    properties: method.params && method.params
